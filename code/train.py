...
parser.add_argument('--data-str', type=str, default='data-mining', help='type of data.')
...
def gae_for(args, seed_num):
    ...
    print("Using {} data".format(args.data_str))
    adj, features = load_data(args.data_str)
    ...
    # Split the dataset
    df = pd.read_csv(
        r'D:\Code\PycharmProjects\LK_project\MHAVGAE-main\2023-10-15\实验数据\AL-CPL\Data-mining\wLabel\ind.data-mining.up.graph',
        header=None)
    df1 = pd.read_csv(
        r'D:\Code\PycharmProjects\LK_project\MHAVGAE-main\2023-10-15\实验数据\AL-CPL\Data-mining\wLabel\ind.data-mining02.up.graph',
        header=None)
    df2 = pd.read_excel(
        r'D:\Code\PycharmProjects\LK_project\MHAVGAE-main\2023-10-15\data\EN-AL-CPL(数据集划分)\dm\test_data.xlsx')

    df3 = pd.read_csv(
        r'D:\Code\PycharmProjects\LK_project\MHAVGAE-main\2023-10-15\实验数据\AL-CPL\Data-mining\wLabel\ind.data-mining.down.graph',
        header=None)
    df4 = pd.read_csv(
        r'D:\Code\PycharmProjects\LK_project\MHAVGAE-main\2023-10-15\实验数据\AL-CPL\Data-mining\wLabel\ind.data-mining02.down.graph',
        header=None)
    ...
    # Some preprocessing
    # t_adj_norm = preprocess_graph(t_adj) # Normalized
    adj_norm_up = preprocess_graph(adj_up)  # Normalized
    ...
    adj_label_up = adj_train_up + sp.eye(adj_train_up.shape[0])
    adj_label_down = adj_train_down + sp.eye(adj_train_down.shape[0])
