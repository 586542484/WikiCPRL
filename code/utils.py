...
# 先用pandas读入csv
data = pd.read_csv(r"D:\Code\PycharmProjects\LK_project\MHAVGAE-main\data\BERT\feature_vector\dm-bert.csv")
b = csr_matrix(data)

def load_data(dataset):
	df = pd.read_csv('data/ind.data-mining.graph', header=None, dtype=int)
    G_matrix = df.values
...
def divide_dataset(df_train, df_train2, df2):
    ...
    test_edges_false_idx = list_0_idx[:len(list_1)]

    test_edges = list_1
    test_edges_false = list_0[test_edges_false_idx]  # 这行是在测试集中选取了和正例相同数量的负例
...
def get_roc_score(emb, adj_orig, edges_pos, edges_neg):
    ...
    for e in edges_pos:
        ...
        if adj_rec[e[1], e[0]] > adj_rec[e[0], e[1]]:
            preds.append(sigmoid(adj_rec[e[1], e[0]]))
        else:
            preds.append(0)
    ...
    for e in edges_neg:
        ...
        if adj_rec[e[1], e[0]] > adj_rec[e[0], e[1]]:
            preds_neg.append(sigmoid(adj_rec[e[1], e[0]]))
        else:
            preds_neg.append(0)
